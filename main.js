// מחשבון משקל פרופילי ברזל - JavaScript פשוט, ללא React

// כל מאגר הנתונים, כל הפרופילים!
const steelData = {
  pipes: {
    name: "צינורות",
...
function getCategories() {
  return Object.keys(steelData).map(key => ({
    key,
    name: steelData[key]?.name || 'לא ידוע'
  }));
}

function getSizesForCategory(category) {
  if (!category || !steelData[category]) return [];
  const categoryData = steelData[category];
  if (categoryData.items) {
    return categoryData.items.map(item => item.size);
  } else if (categoryData.sizes) {
    return categoryData.sizes;
  }
  return [];
}

function getThicknessesForCategory(category, size) {
  if (!category || !steelData[category]) return [];
  const categoryData = steelData[category];
  if (categoryData.weights) {
    const sizeIndex = categoryData.sizes.indexOf(size);
    if (sizeIndex !== -1) {
      return Object.keys(categoryData.weights).filter(thickness => {
        const weights = categoryData.weights[thickness];
        return weights && weights[sizeIndex] !== null && weights[sizeIndex] !== undefined;
      });
    }
  }
  if (category === 'channels') {
    const filtered = categoryData.items.filter(item => item.size === size);
    return [...new Set(filtered.map(item => item.thickness))];
  }
  return [];
}

function getWeight(category, size, thickness = null) {
  if (!category || !size || !steelData[category]) return null;
  const categoryData = steelData[category];
  if (categoryData.items && !thickness) {
    const item = categoryData.items.find(item => item.size === size);
    return item ? item.weight : null;
  }
  if (category === 'channels' && thickness && categoryData.items) {
    const item = categoryData.items.find(item =>
      item.size === size && item.thickness === thickness
    );
    return item ? item.weight : null;
  }
  if (categoryData.weights && thickness && categoryData.sizes) {
    const weights = categoryData.weights[thickness];
    if (weights) {
      const sizeIndex = categoryData.sizes.indexOf(size);
      if (sizeIndex !== -1 && weights[sizeIndex] !== null) {
        return weights[sizeIndex];
      }
    }
  }
  if (categoryData.weights && !thickness) {
    return null;
  }
  return null;
}

function createDescription(category, size, weight, thickness = null) {
  if (!category || !size || !steelData[category]) return '';
  const categoryName = steelData[category].name || 'לא ידוע';
  let description = `${categoryName} ${size}`;
  if (thickness) {
    description += ` עובי ${thickness}מ"מ`;
  }
  return description;
}
...
function render() {
  ...
  app.innerHTML = `
    <div>
      <h1 style="text-align:center;margin-bottom:2rem;">מחשבון משקל פרופילי ברזל</h1>
      
      <div style="margin-bottom:2rem;">
        <label>סוג ברזל:</label>
        <select id="category" style="min-width:140px;">
          <option value="">בחר...</option>
          ${getCategories().map(c => `<option value="${c.key}">${c.name}</option>`).join('')}
        </select>
        
        <label style="margin-right:1.2rem;">גודל:</label>
        <select id="size" style="min-width:140px;" disabled></select>
        
        <label style="margin-right:1.2rem;">עובי דופן:</label>
        <select id="thickness" style="min-width:100px;" disabled></select>
        
        <button id="addItem" disabled style="margin-right:1.2rem;">הוסף לרשימה</button>
      </div>

      <div class="summary-blocks">
        <div class="summary-block">
          <div style="font-size:1.3rem;">מספר פריטים</div>
          <div style="font-size:2rem;">${selectedItems.length}</div>
        </div>
        <div class="summary-block">
          <div style="font-size:1.3rem;">משקל כולל (ק"ג)</div>
          <div style="font-size:2rem;">${totalWeight().toFixed(2)}</div>
        </div>
      </div>
        ...
        <table>
          ...
          <tbody>
            ${selectedItems.length === 0 ? `
              <tr><td colspan="8" style="text-align:center;">לא נוספו פרופילים</td></tr>
            ` : selectedItems.map((item, idx) => `
              <tr>
                <td>${item.categoryName}</td>
                <td>${item.size}</td>
                <td>${item.thickness ? item.thickness + ' מ"מ' : '-'}</td>
                <td><input type="number" min="0.1" step="0.1" style="width:60px;" value="${item.length}" data-id="${item.id}" data-field="length"></td>
                <td><input type="number" min="1" step="1" style="width:60px;" value="${item.quantity}" data-id="${item.id}" data-field="quantity"></td>
                <td>${item.weight}</td>
                <td>${(item.weight * item.length * item.quantity).toFixed(2)}</td>
                <td><button class="removeItemBtn" data-id="${item.id}">X</button></td>
              </tr>
            `).join('')}
          </tbody>
        </table>
      ...
      <div style="margin-top:2rem;text-align:left;">
        <button id="exportBtn" ${selectedItems.length === 0 ? 'disabled' : ''}>ייצוא דוח</button>
      </div>
    </div>
  `;
  ...
  // מילוי גדלים בעת שינוי קטגוריה
  categorySel.onchange = function() {
    const cat = categorySel.value;
    sizeSel.innerHTML = `<option value="">בחר...</option>`;
    thicknessSel.innerHTML = `<option value="">בחר...</option>`;
    thicknessSel.disabled = true;
    addItemBtn.disabled = true;

    if (cat) {
      getSizesForCategory(cat).forEach(size => {
  ...
  sizeSel.onchange = function() {
    ...
    addItemBtn.disabled = true;
    if (cat && size) {
      const thicknesses = getThicknessesForCategory(cat, size);
      if (thicknesses.length > 0) {
        thicknessSel.disabled = false;
        thicknesses.forEach(thick => {
          thicknessSel.innerHTML += `<option value="${thick}">${thick} מ"מ</option>`;
        });
      } else {
        thicknessSel.disabled = true;
  ...
  // אפשרות להפעיל כפתור הוספה אם הכל נבחר
  thicknessSel.onchange = function() {
    addItemBtn.disabled = !categorySel.value || !sizeSel.value || (thicknessSel.disabled ? false : !thicknessSel.value);
  };
  ...
  addItemBtn.onclick = function() {
    const cat = categorySel.value;
    const size = sizeSel.value;
    const thickness = !thicknessSel.disabled ? thicknessSel.value : null;
    if (!cat || !size || (getThicknessesForCategory(cat, size).length > 0 && !thickness)) return;

    const weight = getWeight(cat, size, thickness);
    if (!weight) return;

    const item = {
      id: Date.now() + Math.random(),
      category: cat,
      categoryName: steelData[cat].name,
      size: size,
      thickness: thickness,
      weight: weight,
      length: 1,
  ...
  // שינויי אורך וכמות בטבלה
  document.querySelectorAll('input[type=number][data-id]').forEach(input => {
    input.onchange = function() {
      const id = parseFloat(input.getAttribute('data-id'));
      const field = input.getAttribute('data-field');
      let val = parseFloat(input.value);
      if (isNaN(val) || val <= 0) val = field === "length" ? 1 : 1;
      selectedItems = selectedItems.map(it => it.id === id ? { ...it, [field]: val } : it);
      render();
    }
  });
  ...
  if (exportBtn) {
    exportBtn.onclick = function() {
      let content = "דוח חישוב משקל פרופילי ברזל\n";
      content += "=====================================\n\n";
      content += `תאריך: ${new Date().toLocaleDateString('he-IL')}\n`;
      content += `שעה: ${new Date().toLocaleTimeString('he-IL')}\n\n`;

      content += "פירוט הפריטים:\n";
      content += "--------------\n\n";

      selectedItems.forEach((item, index) => {
        const itemTotal = item.weight * item.quantity * item.length;
        content += `${index + 1}. ${createDescription(item.category, item.size, item.weight, item.thickness)}\n`;
        if (item.thickness) {
          content += `   עובי: ${item.thickness} מ"מ | `;
        }
        content += `אורך: ${item.length} מ' | כמות: ${item.quantity} | `;
        content += `משקל יחידה: ${item.weight} ק"ג/מ' | סך הכל: ${itemTotal.toFixed(2)} ק"ג\n\n`;
      });

      content += `משקל כולל: ${totalWeight().toFixed(2)} ק"ג (${(totalWeight()/1000).toFixed(3)} טון)\n`;
      content += `מספר פריטים: ${selectedItems.length}\n`;

      // יצירת קובץ טקסט
      const element = document.createElement('a');
      const file = new Blob([content], { type: 'text/plain;charset=utf-8' });
      element.href = URL.createObjectURL(file);
      element.download = `דוח_משקל_ברזל_${new Date().toISOString().split('T')[0]}.txt`;
...
function totalWeight() {
  return selectedItems.reduce((sum, item) =>
    sum + (item.weight || 0) * (item.quantity || 0) * (item.length || 0), 0
  );
}
